...
extern "C" int         getStampStats3(float *, int, int, int, int, double *, double *, double *, double *, double *, double *, double *, int, int, int);


void build_matrix_first(stamp_struct *, float *, double *, double *, double *,int *, int *, int *,  int);
void getStampSig(stamp_struct *stamp, double *vectors, double *kernelSol, float *imNoise, float *m1, float *m2, float *m3, int *mm1, int *mm2, int *mm3, int flag, int *mystamp, int * xstamp,int * ystamp);
...
extern "C" void freeStampMem(stamp_struct *stamps,  double *vectors,  double *mat, double *scprod, int nStamps) {
   /*****************************************************
...
extern "C" void cuda_init(float *tRData, float *iRData, float **dtRData, float **diRData){
   ncomp1  = nCompKer - 1;
   ...
   CUDA_SAFE_CALL(cudaMemcpy(*dtRData, tRData, sizeof(float)*rPixX*rPixY, cudaMemcpyHostToDevice));
   CUDA_SAFE_CALL(cudaMemcpy(*diRData, iRData, sizeof(float)*rPixX*rPixY, cudaMemcpyHostToDevice));
   CUDA_SAFE_CALL(cudaMalloc(&d_wxy, sizeof(double)*nStamps*ncomp2));
   CUDA_SAFE_CALL(cudaMalloc(&d_matrix, sizeof(double)*(mat_size+1)*(mat_size+1)));
...
void filter(double * filter_x, double * filter_y, int *ren, int n, int deg_x, int deg_y, int ig) {
   /*****************************************************
...
extern "C" void getKernelVec() {
   ...
   CUDA_SAFE_CALL( cudaMallocHost(&filter_x, sizeof(double)*nCompKer*fwKernel));
   CUDA_SAFE_CALL( cudaMallocHost(&filter_y, sizeof(double)*nCompKer*fwKernel));
   ...
   CUDA_SAFE_CALL( cudaMalloc(&dfilter_x, sizeof(double)*nCompKer*fwKernel));
   CUDA_SAFE_CALL( cudaMalloc(&dfilter_y, sizeof(double)*nCompKer*fwKernel));
  

   CUDA_SAFE_CALL(cudaMemcpy(dig,ig , sizeof(int)*nCompKer, cudaMemcpyHostToDevice));
   CUDA_SAFE_CALL(cudaMemcpy(didegx, idegx, sizeof(int)*nCompKer, cudaMemcpyHostToDevice));
   CUDA_SAFE_CALL(cudaMemcpy(didegy, idegy, sizeof(int)*nCompKer, cudaMemcpyHostToDevice));
   CUDA_SAFE_CALL(cudaMemcpy(dren, ren, sizeof(int)*nCompKer, cudaMemcpyHostToDevice));
   CUDA_SAFE_CALL(cudaMemcpy(dfilter_x, filter_x, sizeof(double)*nCompKer*fwKernel, cudaMemcpyHostToDevice));
   CUDA_SAFE_CALL(cudaMemcpy(dfilter_y, filter_y, sizeof(double)*nCompKer*fwKernel, cudaMemcpyHostToDevice));
...
__global__ void gxy_conv_stamp1(float *dtemp, double *dfilter_y, float *dimage,  int *d_xstamp, int *d_ystamp, int hwKSStamp, int hwKernel,int fwKSStamp,int fwKernel, int rPixX) {
   ...
   int yc;
   imc=0.0;
   __shared__ double filter_y[D_HWKERNEL*2+1];
   __shared__ float image[(D_HWKERNEL*2+1+D_HWKSSTAMP*2+1)/4][(D_HWKSSTAMP*2+1)*2];
   if( threadIdx.x<(2*hwKernel+1) && threadIdx.y==0 )
       filter_y[threadIdx.x]=dfilter_y[threadIdx.x+(int)(blockIdx.x * 0.25)*fwKernel];
   __syncthreads();
...
__global__ void gxy_conv_stamp2(double *vectors, float *dtemp, double *filter_x, int hwKernel,int fwKSStamp,int fwKernel ,int versize) {
...
__global__ void  vector_fix(int *ren ,double *vectors, int fwKSStamp,int versize){
   int m = blockIdx.x *0.25;
   int l = (blockIdx.x %4) * SIZE/4;
   double q;
   if(ren[m]==1){
      q= vectors[threadIdx.x+ l + (m+blockIdx.y *versize)*SIZE] -vectors[threadIdx.x + l + blockIdx.y *versize*SIZE];
      vectors[threadIdx.x+ l + (m+blockIdx.y *versize)*SIZE]=q;
   }
}


__global__ void fill_vec(double *vectors, int *xstamp, int *ystamp,  float rPixX2, float rPixY2, int nCompKer, int fwKSStamp,int hwKSStamp, int vsize, int bgOrder) {
  double xf= (float)(xstamp[blockIdx.y] + threadIdx.x - hwKSStamp - rPixX2) / rPixX2;
  double yf= (float)(ystamp[blockIdx.y] + blockIdx.x - hwKSStamp - rPixY2) / rPixY2;
  int i,j;
  int nv = nCompKer;
  double ax,ay;
         ax = 1.0;    
         for (i = 0; i <= bgOrder; i++) {
	       ...
	       vectors[threadIdx.x + blockIdx.x * fwKSStamp + nv * SIZE+ blockIdx.y * vsize * SIZE] = ax * ay;              
               ay *= yf;
               ++nv;
            }
            ax *= xf;
...
__global__ void build_matrix0(double *mat, double *vectors,int vsize, int half, int pixStamp, int msize ) {
   /*****************************************************
...
__global__ void build_scprod0(double *scprod,double *vec, float *image, int *xstamp, int *ystamp, int fwKSStamp, int hwKSStamp, int rPixX, int nvec,int nC) {
   /*****************************************************
   ...
   for (xc = -hwKSStamp; xc <= hwKSStamp; xc++) {
	    ...
	    p0 += vec[i*SIZE+k+n*SIZE*nvec] *(double) image[xc+xi+rPixX*(yc+yi)];
	    }
	  }
 ...
 extern "C" int fillStamp(stamp_struct *stamp, double *vectors, double *mat, double *scprod, float *imConv, float *imRef, float *image, int *xstamp,int *ystamp, int nStamps) {
   /*****************************************************
   ...
   for (k = 0; k < nStamps; k++) {
     if (verbose >= 1)
        fprintf(stderr, "    xs  : %4i ys  : %4i sig: %6.3f sscnt: %4i nss: %4i \n",
	      stamp[k].x, stamp[k].y, stamp[k].chi2, stamp[k].sscnt, stamp[k].nss);
    ...
    CUDA_SAFE_CALL( cudaMalloc(&dtemp, sizeof(float)*nStamps*nCompKer*SIZE*2));
    CUDA_SAFE_CALL( cudaMemset(dtemp,0, sizeof(float)*nStamps*nCompKer*SIZE*2));
    CUDA_SAFE_CALL( cudaMemset(vectors,0, sizeof(double)*nStamps*vsize*SIZE));
...
extern "C" double check_stamps(stamp_struct *stamps, double *vectors, double *mat, double *scprod, int nStamps, float *imRef, float *imNoise) {
   /*****************************************************
...
CUDA_SAFE_CALL( cudaMallocHost(&temp_matrix, sizeof(double)* nStamps*mats*mats));
CUDA_SAFE_CALL(cudaMemcpy(temp_matrix, mat,sizeof(double)*nStamps*mats*mats, cudaMemcpyDeviceToHost));
CUDA_SAFE_CALL( cudaMallocHost(&testKerSol, sizeof(double)* nStamps*mats));
CUDA_SAFE_CALL(cudaMemcpy(testKerSol, scprod,sizeof(double)*nStamps*mats, cudaMemcpyDeviceToHost));
...
#pragma omp parallel for private( sum)  
 for (i = 0; i < nStamps; i++) {
      ...
      ludcmp_d1(temp_matrix+i*mats*mats, mats-3, mats,indx+i*mats);
      lubksb_d1(temp_matrix+i*mats*mats, mats-3, mats, indx+i*mats, testKerSol+i*mats);
   ...
   if ((strncmp(forceConvolve, "b", 1)==0)) {
      ...
      CUDA_SAFE_CALL( cudaMemcpy(d_sflag, mystamp, sizeof(int)*ntestStamps, cudaMemcpyHostToDevice)); 
         
      /* finally do fit */
      if (verbose >= 2) fprintf(stderr, " Expanding Test Matrix For Fit\n");

      build_matrix_first(stamps, imRef, vectors, mat, scprod,d_sflag,xstamp, ystamp,  ntestStamps);

      CUDA_SAFE_CALL(cudaMemcpy(temp_matrix, d_matrix,(mat_size+1)*sizeof(double)*(mat_size+1), cudaMemcpyDeviceToHost));
      CUDA_SAFE_CALL(cudaMemcpy(testKerSol, d_ksol, sizeof(double)*(nCompTotal+1), cudaMemcpyDeviceToHost));
      ...
      double *temp_kvec=(double*)malloc((sizeof(double)* nCompKer *SIZE));   
      CUDA_SAFE_CALL(cudaMemcpy(temp_kvec, dkernel_vec,(sizeof(double)* nCompKer *SIZE), cudaMemcpyDeviceToHost));
      for (i = 0; i <  nCompKer; i++) {
      kernel_vec[i] = (double *)malloc(fwKernel*fwKernel*sizeof(double));
      ...
      getStampSig(stamps ,vectors,  testKerSol , imNoise, m1, m2, m3, &mcnt1,&mcnt2,&mcnt3,ntestStamps, mystamp, xstamp, ystamp);
...
__global__ void gmake_model(double *vector, double *d_xf, double *d_yf, double *kernelSol, float *csModel, int * goodstamp, int nCompKer, int kerOrder, int mat_size, int vsize) {
   /*****************************************************
   ...
   yf=d_yf[blockIdx.x];
   coeff  = kernelsol[1];
   csModel[threadIdx.x + blockIdx.x * SIZE] += coeff * vector[threadIdx.x +n * vsize];
   ...
   for (i1 = 1; i1 < nCompKer; i1++) {
     // vector = stamp->vectors[i1];
      ...
      for (ix = 0; ix <= kerOrder; ix++) {
         ...
         csModel[threadIdx.x + blockIdx.x * SIZE] += coeff*vector[ threadIdx.x + i1 * SIZE+ n * vsize];
   }
}


void getStampSig(stamp_struct *stamp, double * vectors, double *kernelSol, float *imNoise, float *m1, float *m2, float *m3, int *mm1, int *mm2, int *mm3, int flag, int *mystamp, int * xstamp,int * ystamp) {
   int i, j, idx,  nsig, xRegion, yRegion, xRegion2, yRegion2,mcnt1,mcnt2,mcnt3;
   ...
   CUDA_SAFE_CALL(cudaMemcpy(d_ksol, kernelSol, sizeof(double)*(mat_size+1), cudaMemcpyHostToDevice));   
   ...
   CUDA_SAFE_CALL(cudaMemcpy(whole_temp, d_temp, sizeof(float)*nStamps*SIZE, cudaMemcpyDeviceToHost));  
...
__global__ void build_wxy(double * d_fx, double * d_fy,  double* wxy, int kerOrder, int ncomp2, float rPixX2, float rPixY2){
      const int i =  threadIdx.x;   
      int k,ideg1,ideg2;
      double a1, a2, fx, fy;
      fx     = d_fx[i]; 
...
__global__  void gbuild_scprod( double* wxy, double* vec, float * imRef, double * scprod, double* kernelSol, int  *xstamp, int *ystamp, int *goodstamps,int flag, int vsize, int rPixX, int hwKSStamp,int ncomp1,int ncomp2, int fwKSStamp, int msize){
     ...
     for(s=0;s<flag;s++){   
        ...
        if(threadIdx.x>1 && threadIdx.x<ncomp1*ncomp2+2 ){  //2			
           kernelSol[threadIdx.x] += scprod[i1+2+is*msize] * wxy[s*ncomp2+i2];
	       __syncthreads(); 
        }
...
__global__  void gbuild_matrix( double* wxy, double* matrix0,  double* matrix,int matsize,int *goodstamps,int flag, int vsize, int pixStamp,int ncomp1,int ncomp2, int msize, int fwKSStamp){
     ...
     for(s=0;s<flag;s++){
        ...
        if(i>1 && i<(ncomp1*ncomp2+2)  && j>1 &&  j<=i){  //1
             ...
             matrix[i+j*matsize] += wxy[s*ncomp2+i2] * wxy[s*ncomp2+j2] * matrix0[is*msize*msize+(i1+2)*msize+j1+2];
             matrix[j+i*matsize] += wxy[s*ncomp2+i2] * wxy[s*ncomp2+j2] * matrix0[is*msize*msize+(i1+2)*msize+j1+2];
        ...
        if (i>1 && i<(ncomp1*ncomp2+2) && j==1)          {//3
            ...
            matrix[i+j*matsize] += wxy[s*ncomp2+i2] * matrix0[is*msize*msize+(i1+2)*msize+1];
            matrix[j+i*matsize] += wxy[s*ncomp2+i2] * matrix0[is*msize*msize+(i1+2)*msize+1];
	       
        }
        if (i> (ncomp1*ncomp2+1) && i<matsize && j>1 && j<(ncomp1*ncomp2+2)){//4

           matrix[i+j*matsize] += matrix0[is*msize*msize+ivecbg*msize+j1+2] * wxy[s*ncomp2+j2];
		   matrix[j+i*matsize] += matrix0[is*msize*msize+ivecbg*msize+j1+2]  * wxy[s*ncomp2+j2];
          
        } 
        if (i> (ncomp1*ncomp2+1) && i<matsize && j==1){//5
		   matrix[i+j*matsize] += matrix0[is*msize*msize+ivecbg*msize+1];
		   matrix[j+i*matsize] +=  matrix0[is*msize*msize+ivecbg*msize+1];
		  
        }
        if (i> (ncomp1*ncomp2+1) && i<matsize  && j>(ncomp1*ncomp2+1) && j<=i){//6

           matrix[i+j*matsize] += matrix0[is*msize*msize+ivecbg*msize+(j+ ncomp1 -ncomp1*ncomp2)] ;
		   matrix[j+i*matsize] +=  matrix0[is*msize*msize+ivecbg*msize+(j+ ncomp1 -ncomp1*ncomp2)] ;
...
__inline__ void build_matrix_first(stamp_struct *stamps, float *imRef, double *vectors, double *mat, double *scprod,int *goodstamps, int *xstamp, int *ystamp, int flag) {
   ...
   CUDA_SAFE_CALL(cudaMemcpy(d_fy, fy, sizeof(double)*flag, cudaMemcpyHostToDevice));
   CUDA_SAFE_CALL(cudaMemcpy(d_xstamp, xstamp, sizeof(int)*flag, cudaMemcpyHostToDevice));
   CUDA_SAFE_CALL(cudaMemcpy(d_ystamp, ystamp, sizeof(int)*flag, cudaMemcpyHostToDevice));
...
__global__ void one_conv_stamp1(float *dtemp, double *filter_y, float *image,  int xi, int yi, int hwKSStamp, int hwKernel,int fwKSStamp,int fwKernel, int rPixX) {
   ...
   if(blockIdx.x%4==0){
       for(yc = -hwKernel; yc <= hwKernel; yc++) {
	       imc +=image[ threadIdx.x + xi - hwKSStamp - hwKernel+rPixX*(threadIdx.y+yi+yc-hwKSStamp)] * filter_y[hwKernel-yc+ n*fwKernel];
 
  }

   __syncthreads();
   dtemp[n*2*SIZE+threadIdx.x + threadIdx.y*(fwKSStamp+fwKernel)]= imc ;
   }
   if(blockIdx.x%4==1){
       for(yc = -hwKernel; yc <= hwKernel; yc++) {
	       imc +=image[ threadIdx.x + blockDim.x+ xi - hwKSStamp - hwKernel+rPixX *( threadIdx.y +yi+yc-hwKSStamp)] * filter_y[hwKernel-yc+ n*fwKernel];
       }
       __syncthreads();
   ...
   if(blockIdx.x%4==3){
       ...
       dtemp[n*2*SIZE+threadIdx.x + blockDim.x *3+threadIdx.y*(fwKSStamp+fwKernel)]= imc ;
   }
}


__global__ void one_conv_stamp2(double *vectors, float *dtemp, double *filter_x,  int hwKernel, int fwKSStamp, int fwKernel ,int versize,int istamp) {
...
__global__ void  one_vector_fix(int *ren ,double *vectors, int fwKSStamp,int versize,int istamp){
   int m = blockIdx.x *0.25;
   int l = (blockIdx.x %4) * SIZE/4;
   double q;
   if(ren[m]==1){
...
__global__ void one_fill_vec(double *vectors, int xi, int yi,  float rPixX2, float rPixY2, int nCompKer, int fwKSStamp,int hwKSStamp, int vsize, int bgOrder, int istamp) {
  ...
  double ax,ay;
         ...
         for (i = 0; i <= bgOrder; i++) {
            ay = 1.0; 
            for (j = 0; j <= bgOrder - i; j++) {               
	       vectors[threadIdx.x + blockIdx.x * fwKSStamp + nv * SIZE+ istamp * vsize * SIZE] = ax * ay;              
               ay *= yf;
               ++nv;
            }
            ax *= xf;
...
__global__ void one_build_matrix0(double *mat, double *vectors,int vsize, int half, int pixStamp, int msize ,int istamp) {
   /*****************************************************
    * Build least squares matrix for each stamp
    *****************************************************/   
   int fwx;
   ...
   //vec[threadIdx.y * vsize + threadIdx.x]=0.0;
   //vec[threadIdx.y * vsize + threadIdx.x+half]=0.0;
   if(blockIdx.x % 4 == 0){
	   for(m=0; m<=pixStamp/half;m++) {
	      fwx=m* half +threadIdx.y; 
          vec[threadIdx.y * vsize + threadIdx.x]=vectors[threadIdx.x*SIZE+fwx+ istamp*SIZE*vsize];
          __syncthreads();

          for (k = 0; k < half ; k++)
		     if(m*half+k<pixStamp)  
	 	         q += vec[k * vsize + threadIdx.x] * vec[k * vsize + threadIdx.y];
	       __syncthreads();
	    }
        mat[threadIdx.x+1+msize*(threadIdx.y+1)+istamp *msize*msize] = q;
		__syncthreads();
   }
   ...
   if(blockIdx.x % 4 == 1){
    	for (m=0; m<=pixStamp/half;m++) {
           ...
           vec[threadIdx.y * vsize + threadIdx.x]=vectors[threadIdx.x*SIZE+fwx+istamp*SIZE*vsize];
           vec[threadIdx.y * vsize + threadIdx.x+half]=vectors[fwx+SIZE*(threadIdx.x+half)+istamp*SIZE*vsize];
           __syncthreads();

	   for (k = 0; k <half ; k++)
		   if(m*half+k<pixStamp)  
		    	q += vec[k * vsize + threadIdx.x+half] * vec[k * vsize + threadIdx.y];
		   __syncthreads();
		}
    	 mat[threadIdx.x+half+1+msize*(threadIdx.y+1)+istamp*msize*msize] = q;
		 __syncthreads();
   }
   ...
   if(blockIdx.x % 4 == 2){
	...
	for (m=0; m<=pixStamp/half;m++) {
	   ...
	   for (k = 0; k < half ; k++)
		 if(m*half+k<pixStamp)  
			 q += vec[k * vsize + threadIdx.x] * vec[k * vsize + threadIdx.y+half];
	         __syncthreads();
	   }
	   mat[(threadIdx.x+1)+msize*(threadIdx.y+half+1)+istamp*msize*msize] = q;
	   __syncthreads();
   }
   ...
   if(blockIdx.x % 4 == 3){
	 ...
	 for (m=0; m<=pixStamp/half;m++) {
	     fwx=m*half+threadIdx.y;
		 vec[threadIdx.y * vsize + threadIdx.x]=vectors[fwx+SIZE*(threadIdx.x+half)+istamp*SIZE*vsize];
		 __syncthreads();
  ...
  mat[(threadIdx.x+half+1)+msize*(threadIdx.y+half+1)+istamp*msize*msize] = q;
  __syncthreads();
  }
}


__global__ void one_build_scprod0(double *scprod,double *vec, float *image, int xi, int yi, int fwKSStamp, int hwKSStamp, int rPixX, int nvec,int nC, int istamp) {
   /*****************************************************
    * Build the right side of each stamp's least squares matrix
    *    stamp.scprod = degree of kernel fit + 1 bg term
    *****************************************************/
   ...
   for (xc = -hwKSStamp; xc <= hwKSStamp; xc++) {
         for (yc = -hwKSStamp; yc <= hwKSStamp; yc++) {
            k   = xc + hwKSStamp + fwKSStamp * (yc + hwKSStamp);
	    p0 += vec[i*SIZE+k+istamp*SIZE*nvec] *(double) image[xc+xi+rPixX*(yc+yi)];
	    }
	  }
...
void getStampSig_all(stamp_struct *stamp, double *vectors, double *kernelSol, int *xstamp, int *ystamp, int *mystamp, int flag,float *imNoise, double *sig1, double *sig2, double *sig3) {
...
#pragma omp parallel for private(  i, j, idx,  nsig, xRegion, yRegion, xRegion2, yRegion2, tdat, idat, ndat, diff, bg ,is) 
   ...
   /* don't do think unless you need to! */
   if (strncmp(figMerit, "v", 1)!=0) {
      if (getStampStats3(whole_temp+n*SIZE, xRegion - hwKSStamp, yRegion - hwKSStamp, fwKSStamp, fwKSStamp,
			 &cSum, &cMean, &cMedian, &cMode, &(sig2[is]), &(sig3[is]), &cLfwhm, 0x0, 0xffff, 5)) {
	sig2[is] = -1;
	sig3[is] = -1;
      }
...
int fillStamp_one(stamp_struct *stamp, double *vectors, double *mat, double *scprod, float *imConv, float *imRef, float *image, int istamp) {
   /*****************************************************
    * Fills stamp->vectors with convolved images, and 
    *   pixel indices multiplied by each other for background fit 
    *****************************************************/
    ...
    if (verbose >= 1)
      fprintf(stderr, "    xs  : %4i ys  : %4i sig: %6.3f sscnt: %4i nss: %4i \n",
	      stamp[istamp].x, stamp[istamp].y, stamp[istamp].chi2, stamp[istamp].sscnt, stamp[istamp].nss);
    ...
    CUDA_SAFE_CALL( cudaMemset(vectors+istamp*vsize*SIZE,0, sizeof(double)*vsize*SIZE));
    CUDA_SAFE_CALL( cudaMemset(mat+istamp*mats*mats,0, sizeof(double)*mats*mats));
...
char check_again(stamp_struct *stamps,  double *vectors, double *mat, double *scprod, double *kernelSol, float *imConv, float *imRef, float *image, float *imNoise, int *mystamp, int *xstamp, int *ystamp, int flag, double *meansigSubstamps, double *scatterSubstamps, int *NskippedSubstamps) {
   /*****************************************************
   ...
   *NskippedSubstamps=0;
   ...
   getStampSig_all(stamps, vectors, kernelSol, xstamp, ystamp, mystamp, flag, imNoise, sig1, sig2, sig3);
  ...
  for (istamp = 0; istamp < nStamps; istamp++) {
	 ...
	 if ((strncmp(figMerit, "v", 1)==0 && (sig1[istamp] == -1)) ||
	    ...
	    /* something went wrong with this one... */
	        ...
	        stamps[istamp].sscnt++;
            fillStamp_one(stamps, vectors, mat, scprod, imConv, imRef, image, istamp);
   ...
   for (istamp = 0; istamp < nStamps; istamp++) {
	 ...
	 if ((stamps[istamp].chi2 - mean) > kerSigReject * stdev) { 
	    if (verbose>=2) fprintf(stderr, "    # %d    xss: %4i yss: %4i sig: %6.3f sscnt: %2i nss: %2i ITERATE substamp (poor sig)\n",
				    istamp,
				    stamps[istamp].xss[stamps[istamp].sscnt],
				    stamps[istamp].yss[stamps[istamp].sscnt],
				    stamps[istamp].chi2,
	    ...
	    stamps[istamp].sscnt++;
	    scnt += (!(fillStamp_one(stamps, vectors, mat, scprod, imConv, imRef, image, istamp)));
...
extern "C" void fitKernel(stamp_struct *stamps, double *vectors, double *mat, double *scprod, float *imRef, float *imConv, float *image, float *imNoise, double *kernelSol, 
  ...
  CUDA_SAFE_CALL( cudaMemcpy(d_sflag, mystamp, sizeof(int)*flag, cudaMemcpyHostToDevice));
   ...
   build_matrix_first(stamps, imRef, vectors, mat, scprod, d_sflag, xstamp, ystamp, flag);
   if (verbose>=2) fprintf(stderr, " Expanding Matrix For Full Fit\n");
   double *temp_matrix;
   CUDA_SAFE_CALL( cudaMallocHost(&temp_matrix,sizeof(double)*(mat_size+1)*(mat_size+1) ));
   ...
   check = check_again(stamps, vectors, mat, scprod, testKerSol , imConv, imRef, image, imNoise, mystamp, xstamp, ystamp, flag, meansigSubstamps, scatterSubstamps, NskippedSubstamps);
   ...
   while(check) {
      ...
      CUDA_SAFE_CALL( cudaMemcpy(d_sflag, mystamp, sizeof(int)*nStamps, cudaMemcpyHostToDevice));
      fprintf(stderr, "\n Re-Expanding Matrix\n");
   ...
   fprintf(stderr, " Sigma clipping of bad stamps converged, kernel determined\n");
      ...
      CUDA_SAFE_CALL(cudaMemcpy(kernelSol, testKerSol, sizeof(double)*(nCompTotal+1), cudaMemcpyHostToDevice));
      cudaFreeHost(temp_matrix);
    free( testKerSol);
   return;
}

__global__ void convolve1(float* d_image,double* d_kernel, float* dcrdata ,int* cMask, int* d_mRData, const int kcStep, const int hwKernel,const int xSize,const int ySize, int nsteps_x, const float kerFracMask) {
        
    int tx =  threadIdx.x + blockIdx.x * kcStep;
    int ty =  threadIdx.y + blockIdx.y * kcStep;       
    __shared__ double subkernel[D_HWKERNEL*2+1][D_HWKERNEL*2+1];
    __shared__ float image[2*(D_HWKERNEL*2+1)][2*(D_HWKERNEL*2+1)];
    int    mbit=0x0;
    double aks=0.0;
    double uks=0.0;
    double q=0; 

    subkernel[threadIdx.x][threadIdx.y]=d_kernel[(blockIdx.y * nsteps_x + blockIdx.x)* SIZE + threadIdx.x + (2*hwKernel+1) * threadIdx.y];
    ...
    if(( tx + 2 * hwKernel ) < xSize && ( ty + 2 * hwKernel ) < ySize){
       for(int i = 0; i <=2*hwKernel; i ++) {
         for(int j = 0; j <= 2*hwKernel; j ++) {
             q+=image[threadIdx.x+i][threadIdx.y+j]*subkernel[2*hwKernel-i][2*hwKernel-j];
...
__global__ void variance_1(float* d_image, float* d_variance, double* d_kernel, float* dcrdata ,int* cMask, int* d_mRData, float* d_vData ,  const int kcStep, const int hwKernel,const int xSize,const int ySize, const int nsteps_x, const float kerFracMask) {
    const int tx =  threadIdx.x+blockIdx.x*kcStep;
    const int ty =  threadIdx.y+blockIdx.y*kcStep;
    __shared__ double subkernel[D_HWKERNEL*2+1][D_HWKERNEL*2+1];
    __shared__ float image[2*(D_HWKERNEL*2+1)][2*(D_HWKERNEL*2+1)];
    __shared__ float vari[2*(D_HWKERNEL*2+1)][2*(D_HWKERNEL*2+1)];
    int    mbit=0x0;
    double aks=0.0;
    double uks=0.0;
    double q=0.0; 
    ...
    if((tx + 2 * hwKernel)<xSize && (ty + 2 * hwKernel)<ySize){ 
            for(int i = 0; i <=2*hwKernel; i ++) {
                 for(int j = 0; j <= 2*hwKernel; j ++) {
                      qv+= vari[threadIdx.x + i][threadIdx.y + j] * subkernel[2 * hwKernel - i][2 * hwKernel - j];
                      q+=image[threadIdx.x+i][threadIdx.y+j]*subkernel[2*hwKernel-i][2*hwKernel-j];
                      mbit  |= cMask[tx+i+xSize*(ty+j)] ;
                      aks  += fabs(subkernel[2*hwKernel-i][2*hwKernel-j]);
                      if (!(cMask[tx+i+xSize*(ty+j)] & 0x80)) {
...
__global__ void variance_2(double * kernel_vec, float* d_image, float* variance, double *d_kercoe, int nCompKer, float* dcrdata ,int* d_cMask, int* d_mRData, float* d_vData ,  const int kcStep, const int hwKernel, const int xSize,int ySize, const int fwKernel, const int nsteps_x, const float kerFracMask) {
...
temp=square-1-i;
                 for(j = 0; j <= 2*hwKernel; j ++) {
                     temp_ker = subkernel[temp-fwKernel*j];
                      ...
                      qv+= var[fx+i+j*fwKernel*2] * double_subker[temp-fwKernel*j];
...
__global__  void get_back(float *dcrdata, double *kernelSol, const int kcStep, const int rPixX, const int rPixY, const int hwKernel, const int  ncompBG, const int bgOrder){
...
__global__ void  make_kernel1( double* xf,double* yf,double *kernelSol,double *d_kercoe,int kerOrder,int nCompKer, int nsteps_x) {
   int    k,ix,iy;
   double ax,ay;
   ...
   d_kercoe[nCompKer *( blockIdx.x + nsteps_x * blockIdx.y )+threadIdx.x] = kernel_coeffs[threadIdx.x];
} 

__global__ void make_kernel2(double* d_kernel,double * kernel_vec, double *d_kercoe, int nCompKer, int pixKernel){
...
extern "C"  void spatial_convolve(float *d_image, float * image, float **variance, int xSize, int ySize, double ** kernel_sol, float *cRdata, int *cMask) {
   /*****************************************************
    * Take image and convolve it using the kernelSol every kernel width
    *****************************************************/

   int       l,k,i,j,i2,j2,ni,mbit,jc,jk,ic,ik,nc,i1,j1,nsteps_x,nsteps_y,i0,j0,dovar=1;
...
cudaFree(d_fx); cudaFree(d_fy);
 ...
 CUDA_SAFE_CALL(cudaMalloc(&d_fy, sizeof(double)*nsteps_y));
   ...
   CUDA_SAFE_CALL(cudaMemcpy(d_fx, fx, sizeof(double)*nsteps_x, cudaMemcpyHostToDevice));

   CUDA_SAFE_CALL(cudaMemcpy(d_fy, fy, sizeof(double)*nsteps_x, cudaMemcpyHostToDevice));
   
   CUDA_SAFE_CALL(cudaMalloc((void**)&d_kercoe, sizeof(double) * nCompKer * nsteps_x * gpupart));    
   CUDA_SAFE_CALL(cudaMemset(d_kercoe, 0, sizeof(double) * nCompKer * nsteps_x * gpupart));    

   CUDA_SAFE_CALL(cudaMalloc((void**)&d_kernel, sizeof(double) * SIZE * nsteps_x * gpupart));
   ...
   CUDA_SAFE_CALL(cudaMemset(dcrdata, 0, sizeof(float)*xSize*(gpupart+1)*kcStep));

   for(int i=0;i<xSize*ySize;i++){var[i]=(*variance)[i];}       
...
#pragma omp parallel for private(k)
   ...
   CUDA_SAFE_CALL( cudaMemcpy(cRdata, dcrdata, sizeof(float)*xSize*gpupart*kcStep, cudaMemcpyDeviceToHost));
   CUDA_SAFE_CALL( cudaMemcpy( mRData, d_mRdata, sizeof(int)*xSize*gpupart*kcStep, cudaMemcpyDeviceToHost));
    ...
    if (dovar) {
     CUDA_SAFE_CALL(cudaMemcpy(vData, d_vData, sizeof(float)*xSize*gpupart*kcStep, cudaMemcpyDeviceToHost));
	cudaFree(d_vData);cudaFree(d_variance);
        free(*variance);
        *variance = vData;
    }
...
extern "C" void cuda_finish(float *dtRData, float *diRData, double *iKerSol, double *tKerSol ){
   CUDA_SAFE_CALL( cudaFree(d_xstamp));
   CUDA_SAFE_CALL( cudaFree(d_ystamp));
