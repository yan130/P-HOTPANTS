...
ivectors = NULL; imat = NULL; iscprod = NULL;iKerSol = NULL;
tvectors = NULL; tmat = NULL; tscprod = NULL; tKerSol = NULL;
   ...
   for (i = 0; i < nR; i++) {
	 ...
	 NskippedSubstamps = 0;
      ...
      if (!(kernelImIn)) {
	   ...
	   allocateStamps( ctStamps, &tvectors, &tmat, &tscprod, &tKerSol, nStamps);
	 }
         if (!(strncmp(forceConvolve, "t", 1)==0)) {
           ...
           allocateStamps( ciStamps, &ivectors, &imat, &iscprod, &iKerSol, nStamps); 
	 ...
	 while ((status <= 2) && (flag)) {
	   ...
	   #pragma omp parallel for private (k, ntS, niS, sXMin, sYMin, sXMax, sYMax) 
	    for (l = 0; l < nStampY; l++) {
		  ...
		  ntS=l*10+k;
                  niS=l*10+k;
		  if (verbose >= 2)  fprintf(stderr, "Build stamp  : t %4d i %4d (grid coord %2d %2d)\n", ntS, niS, k, l);
...
//		{
	 fprintf(stderr, "Filling Template sub-stamps\n");
         ...
         if (!(strncmp(forceConvolve, "i", 1)==0)) {
            ...
            fillStamp(ctStamps, tvectors, tmat, tscprod, dtRData, diRData,iRData, xstamp, ystamp, ntS);
            if ((strncmp(forceConvolve, "b", 1)==0)) {
	       fprintf(stderr, "\n\nTrying to convolve the TEMPLATE to fit IMAGE\n");
               tMerit = check_stamps(ctStamps,  tvectors, tmat, tscprod, ntS, diRData, oRData);
	 ...
	 fprintf(stderr, "Filling Image sub-stamps\n");
         if (!(strncmp(forceConvolve, "t", 1)==0)) {
            ...
            for (k = 0; k < niS; k++) {
			 ...
			 fillStamp(ciStamps, ivectors,  imat, iscprod, diRData,dtRData, tRData, xstamp, ystamp, ntS);
            if ((strncmp(forceConvolve, "b", 1)==0)) {
	       fprintf(stderr, "\n\nTrying to convolve the IMAGE to fit TEMPLATE \n");
               iMerit = check_stamps(ciStamps, ivectors, imat, iscprod, niS, tRData, oRData);
               fprintf(stderr, "    Result : merit = %.3f\n", iMerit);
            }
            else
               iMerit = tMerit = 0;
...
/* decide which way to go */
	 ...
	 /* and oRData is the input noise^2 */
         if (!(kernelImIn))
            fitKernel(ctStamps, tvectors,  tmat, tscprod, diRData, dtRData, iRData, oRData, tKerSol, &meansigSubstamps, &scatterSubstamps, &NskippedSubstamps);
	 ...
	 /* spatial_convolve effectively spreads the input mtsRData mask into global mRData output mask!  bitwise... */
         fprintf(stderr, "\n Convolving1...\n");
         spatial_convolve(dtRData, tRData, &eRData, rPixX, rPixY, &tKerSol, oRData, mtsRData);
	 ...
	 /* and oRData is the input noise^2 */
         if (!(kernelImIn))
            fitKernel(ciStamps, ivectors,  imat, iscprod, dtRData, diRData, tRData, oRData, iKerSol, &meansigSubstamps, &scatterSubstamps, &NskippedSubstamps);
...
/* zero out oRData to accept output diff image */
	 ...
	 /* spatial_convolve effectively spreads the input misRData mask into global mRData output mask!  bitwise... */
         fprintf(stderr, "\n Convolving...\n");
         spatial_convolve(diRData,iRData,  &eRData, rPixX, rPixY, &iKerSol, oRData, misRData);  
endTime = omp_get_wtime();
printf("convolve: %.4g\n",endTime - startTime);
