...
int main(int argc, char *argv[]) {
	...
	int *xstamp, *ystamp;
	float *dtRData = NULL, *diRData = NULL;
	/*
	...
	double *ivectors, *imat, *iscprod, *iKerSol;
	double *tvectors, *tmat, *tscprod, *tKerSol;
	ivectors = NULL;
	imat = NULL;
	iscprod = NULL;
	iKerSol = NULL;
	...
	double sum, mean, median, mode, sd, fwhm, lfwhm, diffrat = 0, x2norm;
	...
	if (tNoiseIm) {
		if (fits_open_file(&ePtr, tNoiseIm, 0, &status))
			printError(status);
		if (fits_get_img_param(ePtr, MAXDIM, &oBitpix, &oNaxis, oNaxes,
				&status))
			printError(status);

		if ((oNaxes[0] != tNaxes[0]) || (oNaxes[1] != tNaxes[1])) {
			fprintf(stderr,
					"WARNING : Input template noise array not same size as template, ignoring...\n");
			tNoiseIm = NULL;
		}
	...
	if (iNoiseIm) {
		...
		if ((oNaxes[0] != iNaxes[0]) || (oNaxes[1] != iNaxes[1])) {
			fprintf(stderr,
					"WARNING : Input image noise array not same size as image, ignoring...\n");
			iNoiseIm = NULL;
		}
		if (fits_close_file(ePtr, &status))
			printError(status);
	...
	if (tNoiseIm) {
		...
		if ((oNaxes[0] != iNaxes[0]) || (oNaxes[1] != iNaxes[1])) {
			fprintf(stderr,
					"WARNING : Input template noise array not same size as image, ignoring...\n");
			tNoiseIm = NULL;
		}
		if (fits_close_file(ePtr, &status))
			printError(status);
	...
	if (iMaskIm) {
		...
		if ((oNaxes[0] != iNaxes[0]) || (oNaxes[1] != iNaxes[1])) {
			fprintf(stderr,
					"WARNING : Input image mask array not same size as image, ignoring...\n");
			iMaskIm = NULL;
		}
		if (fits_close_file(ePtr, &status))
			printError(status);
	...
	if (tMaskIm) {
		...
		if ((oNaxes[0] != iNaxes[0]) || (oNaxes[1] != iNaxes[1])) {
			fprintf(stderr,
					"WARNING : Input template mask array not same size as image, ignoring...\n");
			tMaskIm = NULL;
		}
		if (fits_close_file(ePtr, &status))
			printError(status);
	...
	fits_write_key_str(oPtr, "SOFTNAME", "HOTPanTS",
			"The software that differenced this image", &status);
	fits_write_key_str(oPtr, "SOFTVERS", version, "Version", &status);
	fits_write_key_str(oPtr, "SOFTAUTH",
			"Andrew Becker <becker@astro.washington.edu>", "Maintainer",
			&status);
	fits_write_key_str(oPtr, "SOFTINST", "University of Washington",
			"Institute", &status);
	fits_write_key_str(oPtr, "COMMENT", "", "", &status);
	...
	if (!(pstr = getenv("USERNAME"))) /* Cygwin,... */
		if ((pstr = getenv("LOGNAME"))) /* Linux,... */
			fits_write_key_str(oPtr, "AUTHOR", pstr, "Who ran the software",
					&status);
	if (!pstr)
		fits_write_key_str(oPtr, "AUTHOR", "unknown", "Who ran the software",
				&status);
	...
	thetime = time(NULL);
	tm = gmtime(&thetime);
	sprintf(hInfo, "%04d-%02d-%02dT%02d:%02d:%02d", tm->tm_year + 1900,
			tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec);
	fits_write_key_str(oPtr, "DATE", hInfo, "When it was started (GMT)",
			&status);
	...
	if (inclConvImage) {
		if (fits_insert_img(oPtr, oBitpix, oNaxis, oNaxes, &status)
				|| fits_update_key(oPtr, TSTRING, "OBJECT", "Convolved Image",
						"", &status))
			printError(status);
	}
	if (convImage) {
		if (!noClobber) {
			sprintf(scrStr, "!%s", convImage);
		} else {
			sprintf(scrStr, "%s", convImage);
		}
		if (fits_create_file(&ePtr, scrStr, &status)
				...
				|| fits_update_key(ePtr, TSTRING, "OBJECT", "Convolved Image",
						"", &status) || fits_close_file(ePtr, &status))
			printError(status);
	}

	if (inclSigmaImage) {
		if (fits_insert_img(oPtr, oBitpix, 2, oNaxes, &status)
				|| fits_update_key(oPtr, TSTRING, "OBJECT",
						"Noise-scaled (sigma) Difference Image", "", &status))
			printError(status);
	}
	if (sigmaImage) {
		if (!noClobber) {
			sprintf(scrStr, "!%s", sigmaImage);
		} else {
			sprintf(scrStr, "%s", sigmaImage);
		}
		if (fits_create_file(&ePtr, scrStr, &status)
				|| fits_create_img(ePtr, oBitpix, oNaxis, oNaxes, &status)
				|| hp_fits_copy_header(iPtr, ePtr, &status)
				|| fits_update_key(ePtr, TSTRING, "OBJECT",
						"Noise-scaled (sigma) Difference Image", "", &status)
	...
	if (inclNoiseImage) {
		...
		if (fits_update_key(oPtr, TSTRING, "OBJECT", "HOTPanTS Noise Image", "",
				&status))
			printError(status);
		...
		if (outNShort) {
			if (fits_update_key_flt(oPtr, "BZERO", outNiBzero, -5, "", &status)
					|| fits_update_key_flt(oPtr, "BSCALE", outNiBscale, -5, "",
							&status))
				printError(status);
		}
	}
	if (noiseImage) {
		...
		if (hp_fits_copy_header(iPtr, ePtr, &status)
				|| fits_update_key(ePtr, TSTRING, "OBJECT",
						"HOTPanTS Noise Image", "", &status)
				|| fits_update_key_flt(ePtr, "GAIN", 1., -1,
						"No gain in noise image", &status)
				|| fits_update_key_flt(ePtr, "RDNOISE", 0., -1,
						"No rdnoise in noise image", &status)
				|| fits_write_key_flt(ePtr, "MASKVAL", fillValNoise, -6,
						"Value of Masked Pixels", &status))
			printError(status);

		if (outNShort)
			if (fits_update_key_flt(ePtr, "BZERO", outNiBzero, -5, "", &status)
					|| fits_update_key_flt(ePtr, "BSCALE", outNiBscale, -5, "",
							&status))
				printError(status);
	...
	if (outMask) {
		...
		if (fits_create_file(&ePtr, scrStr, &status)
				...
				|| hp_fits_copy_header(iPtr, ePtr, &status)
				|| fits_update_key(ePtr, TSTRING, "OBJECT",
						"Hotpants output Mask Image", "", &status)
	...
	if (!(temp = (float *) calloc((fwKSStamp + fwKernel) * fwKSStamp,
			sizeof(float)))
	...
	xMax = -1
			+ (gdXmax ?
					imin(imin(tNaxes[0], iNaxes[0]), gdXmax) :
					imin(tNaxes[0], iNaxes[0]));
	yMax = -1
			+ (gdYmax ?
					imin(imin(tNaxes[1], iNaxes[1]), gdYmax) :
					imin(tNaxes[1], iNaxes[1]));
	...
	if (regFile) {
		...
		if (!(rXMins = (int *) calloc(nR, sizeof(int))) || !(rXMaxs =
				(int *) calloc(nR, sizeof(int)))
				|| !(rYMins = (int *) calloc(nR, sizeof(int))) || !(rYMaxs =
						(int *) calloc(nR, sizeof(int))))
			exit(1);

		nR = 0;
		while (fscanf(rFile, "%d:%d,%d:%d", &rXMin, &rXMax, &rYMin, &rYMax)
				!= EOF) {
		...
		if (!(rXMins = (int *) calloc(numRegKeyWord, sizeof(int))) || !(rXMaxs =
				(int *) calloc(numRegKeyWord, sizeof(int))) || !(rYMins =
				(int *) calloc(numRegKeyWord, sizeof(int))) || !(rYMaxs =
				(int *) calloc(numRegKeyWord, sizeof(int))))
			exit(1);
		...
		for (nR = 0; nR < numRegKeyWord; nR++) {
			...
			if (fits_read_key_str(tPtr, hKeyword, hInfo, scrStr, &status)) {
				...
				sscanf(hInfo, "[%d:%d,%d:%d]", &rXMins[nR], &rXMaxs[nR],
						&rYMins[nR], &rYMaxs[nR]);
		...
		if (!(rXMins = (int *) calloc(nRegX * nRegY, sizeof(int))) || !(rXMaxs =
				(int *) calloc(nRegX * nRegY, sizeof(int))) || !(rYMins =
				(int *) calloc(nRegX * nRegY, sizeof(int))) || !(rYMaxs =
				(int *) calloc(nRegX * nRegY, sizeof(int))))
			exit(1);
	...
	if ((doKerInfo) || (kernelImOut)) {
		...
		if (doKerInfo) {
			...
			if (fits_insert_btbl(oPtr, (nCompTotal + 1), nR, ttype, tform,
					tunit, "Convolution Kernel Information", 0L, &status)
					|| fits_update_key(oPtr, TSTRING, "OBJECT",
							"Convolution Kernel Information", "", &status)
					|| fits_get_hdu_num(oPtr, &kInfoNum))
				printError(status);
		}
		if (kernelImOut) {
			if (!noClobber) {
				sprintf(scrStr, "!%s", kernelImOut);
			} else {
				sprintf(scrStr, "%s", kernelImOut);
			}
			if (fits_create_file(&ePtr, scrStr, &status)
					|| fits_insert_btbl(ePtr, (nCompTotal + 1), nR, ttype,
							tform, tunit, "Convolution Kernel Information", 0L,
							&status)
					|| fits_update_key(ePtr, TSTRING, "OBJECT",
							"Convolution Kernel Information", "", &status)
	...
	for (i = 0; i < nR; i++) {
		...
		fprintf(stderr, "Region %d pixels            : %ld:%ld,%ld:%ld\n", i,
				pixMin[0], pixMax[0], pixMin[1], pixMax[1]);
		fprintf(stderr, " Vector Indices (buffered) : %d:%d,%d:%d\n", rXBMin,
				rXBMax, rYBMin, rYBMax);
		...
		if (!(kernelImIn)) {
			...
			if (!(strncmp(forceConvolve, "i", 1) == 0)) {
				if (verbose >= 2)
					fprintf(stderr, "Allocating stamps...\n");
				if (!(ctStamps = (stamp_struct *) calloc(nStamps,
						sizeof(stamp_struct)))) {
					printf("Cannot Allocate Stamp List\n");
					exit(1);
				}
				allocateStamps(ctStamps, &tvectors, &tmat, &tscprod, &tKerSol,
						nStamps);
			}
			if (!(strncmp(forceConvolve, "t", 1) == 0)) {
				if (!(ciStamps = (stamp_struct *) calloc(nStamps,
						sizeof(stamp_struct)))) {
					printf("Cannot Allocate Stamp List\n");
					exit(1);
				}
				allocateStamps(ciStamps, &ivectors, &imat, &iscprod, &iKerSol,
						nStamps);
		...
		if (fits_read_subset_flt(tPtr, 0, tNaxis, tNaxes, pixMin, pixMax, inc,
				0, tRData, &anynul, &status)
				|| fits_read_subset_flt(iPtr, 0, iNaxis, iNaxes, pixMin, pixMax,
						inc, 0, iRData, &anynul, &status)) {
			printError(status);
		}
		...
		if (iNoiseIm) {
			if (fits_open_file(&ePtr, iNoiseIm, 0, &status))
				printError(status);
			if (fits_read_subset_flt(ePtr, 0, iNaxis, iNaxes, pixMin, pixMax,
					inc, 0, oRData, &anynul, &status)
					|| fits_close_file(ePtr, &status))
				printError(status);
		...
		if (tNoiseIm) {
			if (fits_open_file(&ePtr, tNoiseIm, 0, &status))
				printError(status);
			if (fits_read_subset_flt(ePtr, 0, tNaxis, tNaxes, pixMin, pixMax,
					inc, 0, eRData, &anynul, &status)
					|| fits_close_file(ePtr, &status))
				printError(status);
		...
		if (iMaskIm) {
			if (fits_open_file(&ePtr, iMaskIm, 0, &status))
				printError(status);
			if (fits_read_subset_int(ePtr, 0, tNaxis, tNaxes, pixMin, pixMax,
					inc, 0, misRData, &anynul, &status)
					|| fits_close_file(ePtr, &status))
				printError(status);
		...
		xstamp = (int *) malloc(sizeof(int) * nStamps);
		ystamp = (int *) malloc(sizeof(int) * nStamps);
		if (!(kernelImIn)) {
			/* only do a single loop for now... */
			while ((status <= 2) && (flag)) {
				flag = 0;
#pragma omp parallel for private (k, ntS, niS, sXMin, sYMin, sXMax, sYMax)
				for (l = 0; l < nStampY; l++) {
					for (k = 0; k < nStampX; k++) {
						ntS = l * 10 + k;
						niS = l * 10 + k;
						if (verbose >= 2)
							fprintf(stderr,
									"Build stamp  : t %4d i %4d (grid coord %2d %2d)\n",
									ntS, niS, k, l);
						...
						if (sstampFile) {
							if (verbose >= 2)
								fprintf(stderr, "Adding centers manually\n");
							for (m = 0; m < Ncmp; m++) {
								if ((xcmp[m] > sXMin + hwKernel + 1)
										&& (xcmp[m] < sXMax - hwKernel - 1)
										&& (ycmp[m] > sYMin + hwKernel + 1)
										&& (ycmp[m] < sYMax - hwKernel - 1)) {

									buildStamps(sXMin, sXMax, sYMin, sYMax,
											&niS, &ntS, 0, rXBMin, rYBMin,
							...
							if (findSSC) {
								if (verbose >= 2)
									fprintf(stderr,
											"Automatically finding additional centers\n");
								/* and then choose enough stamps manually to fill the array */
								buildStamps(sXMin, sXMax, sYMin, sYMax, &niS,
										&ntS, 1, rXBMin, rYBMin, ciStamps,
										ctStamps, iRData, tRData, 0, 0);
							}
						} else {
							if (useFullSS) {
								/* don't try and find centers, just use center of stamp */
								buildStamps(sXMin, sXMax, sYMin, sYMax, &niS,
										&ntS, 0, rXBMin, rYBMin, ciStamps,
										ctStamps, iRData, tRData, 0, 0);
							} else {
								buildStamps(sXMin, sXMax, sYMin, sYMax, &niS,
										&ntS, 1, rXBMin, rYBMin, ciStamps,
				...
				for (l = 0; l < nStampY; l++) {
					for (k = 0; k < nStampX; k++) {
						...
						if (!(strncmp(forceConvolve, "i", 1) == 0)) {
							if (verbose >= 2)
								fprintf(stderr, "    templ: %d substamps\n",
										ctStamps[ntS].nss);
							if (ctStamps[l * 10 + k].nss <= 0) {
								ctStamps[l * 10 + k] = ctStamps[ntS];
								ntS--;
							}
						}
						if (!(strncmp(forceConvolve, "t", 1) == 0)) {
							if (verbose >= 2)
								fprintf(stderr, "    image: %d substamps\n",
										ciStamps[niS].nss);
							if (ciStamps[l * 10 + k].nss <= 0) {
								ciStamps[l * 10 + k] = ciStamps[niS];
								niS--;
							}
						}
					}
				...
				if (strncmp(forceConvolve, "i", 1) == 0) {
					...
					if (tSFrac < minFracGoodStamps)
						flag = 1;
				} else if ((iSFrac < minFracGoodStamps)
						|| (tSFrac < minFracGoodStamps)) {
					fprintf(stderr,
							"%d and %d stamps built (%.2f%s, %.2f%s)\n\n", ntS,
							niS, tSFrac, "%", iSFrac, "%");
					flag = 1;
				} else {
					fprintf(stderr,
				...
				if ((flag) && (status <= 1) && (scaleFitThresh < 1.)) {
					...
					fprintf(stderr,
							"Too few stamps were fit, scaling down fitting threshold to %.2f\n",
							kerFitThresh);

					if (ctStamps) {
						freeStampMem(ctStamps, tvectors, tmat, tscprod,
								nStamps);
						free(ctStamps);
						ctStamps = NULL;
					}
					if (ciStamps) {
						freeStampMem(ciStamps, ivectors, imat, iscprod,
								nStamps);
						free(ciStamps);
						ciStamps = NULL;
					}
			...
			cuda_init(tRData, iRData, &dtRData, &diRData);
			getKernelVec();
...
//		{
			...
			if (!(strncmp(forceConvolve, "i", 1) == 0)) {
				for (k = 0; k < ntS; k++) {
					ctStamps[k].sscnt = 0;
					xstamp[k] = ctStamps[k].xss[0];
					ystamp[k] = ctStamps[k].yss[0];
				}
				fillStamp(ctStamps, tvectors, tmat, tscprod, dtRData, diRData,
						iRData, xstamp, ystamp, ntS);
				if ((strncmp(forceConvolve, "b", 1) == 0)) {
					fprintf(stderr,
							"\n\nTrying to convolve the TEMPLATE to fit IMAGE\n");
					tMerit = check_stamps(ctStamps, tvectors, tmat, tscprod,
							ntS, diRData, oRData);
...
//#pragma omp section		{
			...
			if (!(strncmp(forceConvolve, "t", 1) == 0)) {
				...
				for (k = 0; k < niS; k++) {
					ciStamps[k].sscnt = 0;
					xstamp[k] = ciStamps[k].xss[0];
					ystamp[k] = ciStamps[k].yss[0];

				}
				fillStamp(ciStamps, ivectors, imat, iscprod, diRData, dtRData,
						tRData, xstamp, ystamp, ntS);
				if ((strncmp(forceConvolve, "b", 1) == 0)) {
					fprintf(stderr,
							"\n\nTrying to convolve the IMAGE to fit TEMPLATE \n");
					iMerit = check_stamps(ciStamps, ivectors, imat, iscprod,
							niS, tRData, oRData);
					fprintf(stderr, "    Result : merit = %.3f\n", iMerit);
				} else
					iMerit = tMerit = 0;
...
//}
		...
		if (convTmpl) {
			...
			freeStampMem(ciStamps, ivectors, imat, iscprod, nStamps);
			...
			if (!(kernelImIn))
				fitKernel(ctStamps, tvectors, tmat, tscprod, diRData, dtRData,
						iRData, oRData, tKerSol, &meansigSubstamps,
			...
			/* add in my own bits */
			for (l = rPixX * rPixY; l--;) {
				mtsRData[l] |= (FLAG_INPUT_ISBAD | FLAG_BAD_PIXVAL)
						* (tRData[l] == fillVal);
				mtsRData[l] |= (FLAG_INPUT_ISBAD | FLAG_SAT_PIXEL)
						* (tRData[l] >= tUThresh);
				mtsRData[l] |= (FLAG_INPUT_ISBAD | FLAG_LOW_PIXEL)
						* (tRData[l] <= tLThresh);
			}
			...
			if (tNoiseIm) {
				if (fits_open_file(&ePtr, tNoiseIm, 0, &status))
					printError(status);
				if (fits_read_subset_flt(ePtr, 0, tNaxis, tNaxes, pixMin,
						pixMax, inc, 0, eRData, &anynul, &status)
						|| fits_close_file(ePtr, &status))
					printError(status);
			...
			fprintf(stderr, "\n Convolving1...\n");
			spatial_convolve(dtRData, tRData, &eRData, rPixX, rPixY, &tKerSol,
					oRData, mtsRData);
			...
			if (iNoiseIm) {
				if (fits_open_file(&ePtr, iNoiseIm, 0, &status))
					printError(status);
				if (fits_read_subset_flt(ePtr, 0, iNaxis, iNaxes, pixMin,
						pixMax, inc, 0, tRData, &anynul, &status)
						|| fits_close_file(ePtr, &status))
					printError(status);
			...
			for (l = rPixX * rPixY; l--;) {
				mRData[l] |= (FLAG_OUTPUT_ISBAD | FLAG_INPUT_ISBAD
						| FLAG_BAD_PIXVAL) * (iRData[l] == fillVal);
				mRData[l] |= (FLAG_OUTPUT_ISBAD | FLAG_INPUT_ISBAD
						| FLAG_SAT_PIXEL) * (iRData[l] >= iUThresh);
				mRData[l] |= (FLAG_OUTPUT_ISBAD | FLAG_INPUT_ISBAD
						| FLAG_LOW_PIXEL) * (iRData[l] <= iLThresh);
				mRData[l] |= misRData[l];
				mRData[l] |= FLAG_OUTPUT_ISBAD
						* ((misRData[l] & FLAG_INPUT_ISBAD) > 0);
			}
			...
			freeStampMem(ctStamps, tvectors, tmat, tscprod, nStamps);
			/*allocateStamps(ctStamps, nStamps);*/
			nS = niS;
			...
			/* and oRData is the input noise^2 */
			if (!(kernelImIn))
				fitKernel(ciStamps, ivectors, imat, iscprod, dtRData, diRData,
						tRData, oRData, iKerSol, &meansigSubstamps,
						&scatterSubstamps, &NskippedSubstamps);
			...
			/* add in my own bits */
			for (l = rPixX * rPixY; l--;) {
				misRData[l] |= (FLAG_INPUT_ISBAD | FLAG_BAD_PIXVAL)
						* (iRData[l] == fillVal);
				misRData[l] |= (FLAG_INPUT_ISBAD | FLAG_SAT_PIXEL)
						* (iRData[l] >= iUThresh);
				misRData[l] |= (FLAG_INPUT_ISBAD | FLAG_LOW_PIXEL)
						* (iRData[l] <= iLThresh);
			}
			...
			fprintf(stderr, "\n Convolving...\n");
			spatial_convolve(diRData, iRData, &eRData, rPixX, rPixY, &iKerSol,
					oRData, misRData);
			endTime = omp_get_wtime();
			printf("convolve: %.4g\n", endTime - startTime);
			...
			if (tNoiseIm) {
				if (fits_open_file(&ePtr, tNoiseIm, 0, &status))
					printError(status);
				if (fits_read_subset_flt(ePtr, 0, tNaxis, tNaxes, pixMin,
						pixMax, inc, 0, iRData, &anynul, &status)
						|| fits_close_file(ePtr, &status))
					printError(status);
			...
			for (l = rPixX * rPixY; l--;) {
				mRData[l] |= (FLAG_OUTPUT_ISBAD | FLAG_INPUT_ISBAD
						| FLAG_BAD_PIXVAL) * (tRData[l] == fillVal);
				mRData[l] |= (FLAG_OUTPUT_ISBAD | FLAG_INPUT_ISBAD
						| FLAG_SAT_PIXEL) * (tRData[l] >= tUThresh);
				mRData[l] |= (FLAG_OUTPUT_ISBAD | FLAG_INPUT_ISBAD
						| FLAG_LOW_PIXEL) * (tRData[l] <= tLThresh);
				mRData[l] |= mtsRData[l];
				mRData[l] |= FLAG_OUTPUT_ISBAD
						* ((mtsRData[l] & FLAG_INPUT_ISBAD) > 0);
			}
				...
				if (strncmp(figMerit, "v", 1) == 0) {
					...
					for (l = 0; l < nS; l++) {
						...
						if (ctStamps[l].sscnt < ctStamps[l].nss) {
							getFinalStampSig(&ctStamps[l], oRData, tRData,
									&sum);
							temp2[k++] = sum;
					...
					sigma_clip(temp2, k, &meansigSubstampsF, &scatterSubstampsF,
							10);
				...
				if (strncmp(figMerit, "v", 1) == 0) {
					...
					for (l = 0; l < nS; l++) {
						...
						if (ciStamps[l].sscnt < ciStamps[l].nss) {
							getFinalStampSig(&ciStamps[l], oRData, iRData,
									&sum);
							temp2[k++] = sum;
							/*fprintf(stderr, "SSSIG %d : %f\n", k, sum);*/
						}
...
#pragma omp section
			{
				getStampStats3(oRData, 0, 0, rPixX, rPixY, &sum, &mean, &median,
						&mode, &sd, &fwhm, &lfwhm, 0x0, 0xffff, 5);
			}
#pragma omp section
			{
				if (convTmpl) {
					getStampStats3(tRData, 0, 0, rPixX, rPixY, &nsum, &nmean,
							&nmedian, &nmode, &nsd, &nfwhm, &nlfwhm, 0x0,
							0xffff, 5);
					getNoiseStats3(oRData, tRData, &x2norm, &nx2norm, 0x0,
							0xffff);
				}

				else {
					getStampStats3(iRData, 0, 0, rPixX, rPixY, &nsum, &nmean,
							&nmedian, &nmode, &nsd, &nfwhm, &nlfwhm, 0x0,
							0xffff, 5);
					getNoiseStats3(oRData, iRData, &x2norm, &nx2norm, 0x0,
							0xffff);
				}
			}
#pragma omp section
			{
				getStampStats3(oRData, 0, 0, rPixX, rPixY, &summ, &meanm,
						&medianm, &modem, &sdm, &fwhmm, &lfwhmm, 0xff,
						FLAG_OUTPUT_ISBAD, 5);
			}
#pragma omp section
			{
				if (convTmpl)
					getStampStats3(tRData, 0, 0, rPixX, rPixY, &nsumm, &nmeanm,
							&nmedianm, &nmodem, &nsdm, &nfwhmm, &nlfwhmm, 0xff,
							FLAG_OUTPUT_ISBAD, 5);
				else
					getStampStats3(iRData, 0, 0, rPixX, rPixY, &nsumm, &nmeanm,
							&nmedianm, &nmodem, &nsdm, &nfwhmm, &nlfwhmm, 0xff,
							FLAG_OUTPUT_ISBAD, 5);
			}
		}
		...
		if (verbose >= 2)
			fprintf(stderr, "   FWHM   : %.2f\n", fwhm);
		if (verbose >= 2)
			fprintf(stderr, "   lFWHM  : %.2f\n", lfwhm);
		...
		if (!(kernelImIn))
			fprintf(stderr,
					" Emperical / Expected Noise for GOOD pixels = %.2f\n",
					sd / nmean);
		...
		if (!(kernelImIn))
			fprintf(stderr,
					" Emperical / Expected Noise for OK pixels = %.2f\n\n",
					sdm / nmeanm);
		...
		if (inclSigmaImage || sigmaImage) {
			...
			if (inclSigmaImage) {
				if (fits_movrel_hdu(oPtr, 1, NULL, &status)
						|| hp_fits_write_subset(oPtr, 0, 2, oNaxes, oRData,
								&status, outShort, outBzero, outBscale,
		...
		if (inclNoiseImage) {
			...
			if (convTmpl) {
				if (hp_fits_write_subset(oPtr, 0, 2, oNaxes, tRData, &status,
						outNShort, outNiBzero, outNiBscale, fpixelOutX,
						fpixelOutY, lpixelOutX, lpixelOutY, xBufLo, yBufLo))
					printError(status);
			} else {
		...
		if (outMask) {
			if (fits_open_file(&ePtr, outMask, 1, &status))
				printError(status);
			if (hp_fits_write_subset_int(ePtr, 0, 2, oNaxes, mRData, &status, 1,
					32768, 1, fpixelOutX, fpixelOutY, lpixelOutX, lpixelOutY,
					xBufLo, yBufLo) || fits_close_file(ePtr, &status))
				printError(status);
		}
		...
		sprintf(hInfo, "[%d:%d,%d:%d]", rXMin + 1, rXMax + 1, rYMin + 1,
				rYMax + 1);
		...
		fits_write_key_str(oPtr, hKeyword, hInfo,
				"Average Figure of Merit across Stamps", &status);
		if (kernelImOut)
			fits_write_key_str(ePtr, hKeyword, hInfo,
					"Average Figure of Merit across Stamps", &status);

		sprintf(hKeyword, "SSSCAT%02d", i);
		sprintf(hInfo, "%.4f", scatterSubstamps);
		fits_write_key_str(oPtr, hKeyword, hInfo, "Stdev in Figure of Merit",
				&status);
		if (kernelImOut)
			fits_write_key_str(ePtr, hKeyword, hInfo,
					"Stdev in Figure of Merit", &status);
		...
		if (strncmp(figMerit, "v", 1) == 0) {
			...
			if (kernelImOut)
				fits_write_key_str(ePtr, hKeyword, hInfo, "Final SSSIG",
						&status);
			...
			if (kernelImOut)
				fits_write_key_str(ePtr, hKeyword, hInfo, "Final SSSCAT",
						&status);
		...
		fits_write_key_str(oPtr, hKeyword, hInfo, "1/N * SUM (diff/noise)^2",
				&status);
		if (kernelImOut)
			fits_write_key_str(ePtr, hKeyword, hInfo,
					"1/N * SUM (diff/noise)^2", &status);

		sprintf(hKeyword, "NX2NRM%02d", i);
		sprintf(hInfo, "%d", nx2norm);
		fits_write_key_str(oPtr, hKeyword, hInfo, "Number of pixels in X2NRM",
				&status);
		if (kernelImOut)
			fits_write_key_str(ePtr, hKeyword, hInfo,
					"Number of pixels in X2NRM", &status);

		sprintf(hKeyword, "DMEAN%02d", i);
		sprintf(hInfo, "%.4f", mean);
		fits_write_key_str(oPtr, hKeyword, hInfo,
				"Mean of diff image; good pixels", &status);
		if (kernelImOut)
			fits_write_key_str(ePtr, hKeyword, hInfo,
					"Mean of diff image; good pixels", &status);
		...
		sprintf(hInfo, "%.4f", sd);
		fits_write_key_str(oPtr, hKeyword, hInfo,
				"Stdev of diff image; good pixels", &status);
		if (kernelImOut)
			fits_write_key_str(ePtr, hKeyword, hInfo,
					"Stdev of diff image; good pixels", &status);
		...
		fits_write_key_str(oPtr, hKeyword, hInfo,
				"Mean of noise image; good pixels", &status);
		if (kernelImOut)
			fits_write_key_str(ePtr, hKeyword, hInfo,
					"Mean of noise image; good pixels", &status);
		...
		fits_write_key_str(oPtr, hKeyword, hInfo,
				"Mean of diff image; OK pixels", &status);
		if (kernelImOut)
			fits_write_key_str(ePtr, hKeyword, hInfo,
					"Mean of diff image; OK pixels", &status);
		...
		fits_write_key_str(oPtr, hKeyword, hInfo,
				"Stdev of diff image; OK pixels", &status);
		if (kernelImOut)
			fits_write_key_str(ePtr, hKeyword, hInfo,
					"Stdev of diff image; OK pixels", &status);
		...
		fits_write_key_str(oPtr, hKeyword, hInfo,
				"Mean of noise image; OK pixels", &status);
		if (kernelImOut)
			fits_write_key_str(ePtr, hKeyword, hInfo,
					"Mean of noise image; OK pixels", &status);
		...
		if (rescaleOK) {
			...
			sprintf(hInfo, "%.4f", diffrat);
			fits_write_key_str(oPtr, hKeyword, hInfo,
					"Rescaling factor of OK noise pixels", &status);
			if (kernelImOut)
				fits_write_key_str(ePtr, hKeyword, hInfo,
						"Rescaling factor of OK noise pixels", &status);
		}
		...
		if (doKerInfo) {
			if (convTmpl) {
				if (fits_movabs_hdu(oPtr, kInfoNum, NULL, &status)
						|| fits_write_col_dbl(oPtr, i + 1, 1, 1,
								(nCompTotal + 1), tKerSol, &status))
					printError(status);
			} else {
				if (fits_movabs_hdu(oPtr, kInfoNum, NULL, &status)
						|| fits_write_col_dbl(oPtr, i + 1, 1, 1,
								(nCompTotal + 1), iKerSol, &status))
					printError(status);
			}
		}
		if (kernelImOut) {
			if (convTmpl) {
				if (fits_open_file(&ePtr, kernelImOut, 1, &status)
						...
						|| fits_write_col_dbl(ePtr, i + 1, 1, 1,
								(nCompTotal + 1), tKerSol, &status)
						|| fits_close_file(ePtr, &status))
					printError(status);
			} else {
	...
	fits_write_key_longstr(oPtr, "TARGET", image, "HOTPanTS : Input Image",
			&status);
	fits_write_key_longstr(oPtr, "TEMPLATE", template,
	...
	fits_write_key_lng(oPtr, "NREGION", nRegX * nRegY,
			"Number of independent regions", &status);
	fits_write_key_flt(oPtr, "MASKVAL", fillVal, -6, "Value of Masked Pixels",
			&status);
	...
	if (doKerInfo) {
		...
		fits_write_key_lng(oPtr, hKeyword, ngauss,
				"Number of Gaussian Basis Functions", &status);
		for (k = 0; k < ngauss; k++) {
			sprintf(hKeyword, "DGAUSS%d", k + 1);
			fits_write_key_lng(oPtr, hKeyword, deg_fixe[k], "Polynomial Degree",
					&status);

			sprintf(hKeyword, "SGAUSS%d", k + 1);
			fits_write_key_flt(oPtr, hKeyword, 1. / sqrt(2. * sigma_gauss[k]),
					8, "Gaussian Sigma", &status);
		}

		fits_write_key_lng(oPtr, "FWKERN", fwKernel, "Kernel Width in Pixels",
				&status);
		fits_write_key_lng(oPtr, "CKORDER", kerOrder,
				"Spatial Convolution Order", &status);
		fits_write_key_lng(oPtr, "BGORDER", bgOrder, "Background Order",
				&status);
	} else {
		if (!(kernelImOut))
	...
	if (kernelImOut) {
		...
		fits_write_key_str(ePtr, "PHOTNORM", photNormalize,
				"Direction of photometric normalization", &status);

		sprintf(hKeyword, "NGAUSS");
		fits_write_key_lng(ePtr, hKeyword, ngauss,
				"Number of Gaussian Basis Functions", &status);
		fits_write_key_lng(oPtr, hKeyword, ngauss,
				"Number of Gaussian Basis Functions", &status);
		for (k = 0; k < ngauss; k++) {
			sprintf(hKeyword, "DGAUSS%d", k + 1);
			fits_write_key_lng(ePtr, hKeyword, deg_fixe[k], "Polynomial Degree",
					&status);
			fits_write_key_lng(oPtr, hKeyword, deg_fixe[k], "Polynomial Degree",
					&status);

			sprintf(hKeyword, "SGAUSS%d", k + 1);
			fits_write_key_flt(ePtr, hKeyword, 1. / sqrt(2. * sigma_gauss[k]),
					8, "Gaussian Sigma", &status);
			fits_write_key_flt(oPtr, hKeyword, 1. / sqrt(2. * sigma_gauss[k]),
					8, "Gaussian Sigma", &status);
		}

		fits_write_key_lng(ePtr, "FWKERN", fwKernel, "Kernel Width in Pixels",
				&status);
		fits_write_key_lng(ePtr, "CKORDER", kerOrder,
				"Spatial Convolution Order", &status);
		fits_write_key_lng(ePtr, "BGORDER", bgOrder, "Background Order",
				&status);
		fits_write_key_lng(oPtr, "FWKERN", fwKernel, "Kernel Width in Pixels",
				&status);
		fits_write_key_lng(oPtr, "CKORDER", kerOrder,
	...
	if (ycmp)
		free(ycmp);
	cuda_finish(dtRData, diRData, iKerSol, tKerSol);
	endTime = omp_get_wtime();
	printf("end:  %.3g\n", endTime - startTime);
