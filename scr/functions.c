...
extern int cutSStamp(stamp_struct *stamp, float *iData) {
   /*****************************************************
...
#pragma omp parallel for private(ddat, mdat, ndat ) reduction( +:nsum, n) 
   for (i=0; i < rPixX*rPixY;  i++ ) {
      ddat = data[i];
      mdat = mRData[i];
...
extern int sigma_clip(float *data, int count, double *mean, double *stdev, int maxiter) {
   int cnt, ncnt, i, iter;
   char *smask;
   double istdev;
   float d;
   double temp_mean, temp_stdev;
   if (count == 0) {
      *mean  = 0;
      *stdev = MAXVAL;
      return 1;
   ...
   while ((ncnt != cnt) && (iter < maxiter)) {
      ...
      temp_mean  = 0;
      temp_stdev = 0;
//#pragma omp parallel for private(d) reduction(+: temp_mean, temp_stdev)
      for (i=0; i<count; i++) {
	 if (!(smask[i])) {
	    d       = data[i];
	    temp_mean  += d;
	 ...
	 return 2;
      ...
      if (ncnt > 1) {
	 temp_stdev = temp_stdev - ncnt * (temp_mean) * (temp_mean);
	 temp_stdev = sqrt(temp_stdev / (double)(ncnt - 1));
      }
      else {
	 temp_stdev = MAXVAL;
	 free(smask);
	 ...
	 if (!(smask[i])) {
	    ...
	    if ((fabs(data[i] - (temp_mean)) * istdev) > statSig) {
	       smask[i] = 1;
	    }
	    else {
	       ncnt++;
